/**
 * Remove Punctuation API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Remove Punctuation API.
 ...
 */
async function callRemovePunctuationAPIWithErrorHandling(requestData) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callRemovePunctuationAPIWithErrorHandling({
